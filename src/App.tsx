  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'model',
      content: "Assalomu alaykum! Men Paketshop.uz'ning do'stona yordamchisi - Malikaman! Qanday yordam bera olaman? 😊"
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAudioEnabled && fullResponseText) {
          try {
              const base64Audio = await generateSpeech(fullResponseText);
              if (base64Audio) {
                  playPCMBase64(base64Audio, modelMessageId);
              }
          } catch(err) {
